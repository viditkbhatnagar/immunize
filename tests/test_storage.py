...
def test_list_artifacts_orders_by_id_desc(conn: sqlite3.Connection) -> None:
    storage.insert_match(
        conn, slug="a", pattern_id="p-a", pattern_origin="bundled", paths={}, verified=True
    )
    storage.insert_match(
        conn, slug="b", pattern_id="p-b", pattern_origin="local", paths={}, verified=False
    )
    rows = storage.list_artifacts(conn)
    assert [r.slug for r in rows] == ["b", "a"]


def test_delete_artifact_returns_row_then_removes_it(conn: sqlite3.Connection) -> None:
...
def test_slug_exists(conn: sqlite3.Connection) -> None:
    storage.insert_match(
        conn,
        slug="my-slug",
        pattern_id="p",
